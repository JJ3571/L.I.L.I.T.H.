...
class VoiceCog(commands.Cog, CogLogMixin):
    def __init__(self, bot):
        self.bot = bot
        self.reserved_channels = {}  # channel_id: timestamp when reservation ends
        self.cog_print("Initializing VoiceCog.")

    def cog_unload(self):
        self.cog_print("VoiceCog has been unloaded.")
    ...
    async def on_voice_state_update(self, member, before, after):
        ...
        if not hidden_category or not seen_category:
            self.cog_print(f"Categories not found in guild '{guild.name}'.")
            return

        # User joins the create_fireteam channel
        if after.channel and after.channel.id == create_fireteam_channel_id:
            self.cog_print(f"{member.name} has joined the create_fireteam channel.")
            ...
            for channel_id in voice_channel_ids:
                ...
                if channel and channel.category and channel.category.id == hidden_category_id:
                    ...
                    self.cog_print(f"Moved '{channel.name}' to seen category and updated permissions.")
                    break  # Move only one channel

            if moved_channel:
                # Move the member to the newly moved channel
                await member.move_to(moved_channel)
                self.cog_print(f"Moved {member.name} to '{moved_channel.name}'.")
        ...
        if before.channel and before.channel.id in voice_channel_ids:
            if len(before.channel.members) == 0:
                ...
                if reservation_end_time and time.time() < reservation_end_time.timestamp():
                    self.cog_print(f"Channel '{before.channel.name}' is reserved until {reservation_end_time}.")
                else:
    ...
    async def tidy_up(self, interaction: nextcord.Interaction):
        ...
        if not hidden_category or not seen_category:
            ...
            self.cog_print("Categories not found during tidy_up command.")
            return
        ...
        if moved_to_hidden_count > 0:
            ...
            self.cog_print(f"Tidy_up: Moved {moved_to_hidden_count} standard channels to hidden.")
        ...
        if waterboard_cog and hasattr(waterboard_cog, 'get_temp_channels') and hasattr(
            ...
            self.cog_print("Tidy_up: Found waterboard cog with temp-channel DB. Proceeding with waterboard channel cleanup.")
            try:
                ...
                if hasattr(waterboard_cog, 'create_tables') and not hasattr(waterboard_cog, '_tables_created'):
                     if hasattr(waterboard_cog, 'create_tables'): # Check if method exists
                        ...
                        self.cog_print("Tidy_up: Ensured waterboard cog tables are created.")
                ...
                if temp_wb_channel_ids:
                    self.cog_print(f"Tidy_up: Waterboard DB lists channels: {temp_wb_channel_ids}")
                    for wb_channel_id in list(temp_wb_channel_ids):  # Iterate over a copy
                        ...
                        if channel_to_delete and isinstance(channel_to_delete, nextcord.VoiceChannel):
                            self.cog_print(f"Tidy_up: Attempting to delete Discord waterboard channel: {channel_to_delete.name} ({wb_channel_id})")
                            try:
                                await channel_to_delete.delete(reason="Tidy Up command (Waterboard DB)")
                                deleted_wb_discord_channels += 1
                            except nextcord.errors.NotFound:
                                self.cog_print(f"Tidy_up: Waterboard channel {wb_channel_id} already deleted from Discord.")
                            except Exception as e:
                                self.cog_print(f"Tidy_up: Error deleting waterboard channel {wb_channel_id} from Discord: {e}")
                        else:
                            self.cog_print(f"Tidy_up: Waterboard channel {wb_channel_id} from DB not found on Discord or not a voice channel.")
                    ...
                    self.cog_print(f"Tidy_up: Waterboard cleanup complete. Deleted {deleted_wb_discord_channels} Discord channels, removed {deleted_wb_db_entries} DB entries.")
                else:
                    self.cog_print("Tidy_up: No waterboard channels found in the waterboard cog's database.")
            except Exception as e:
                self.cog_print(f"Tidy_up: An error occurred during waterboard channel cleanup: {e}")
                tidied_messages.append("An error occurred during waterboard channel cleanup.")
        elif waterboard_cog:
            self.cog_print("Tidy_up: Waterboard v3 loaded; skipping legacy temp-channel DB cleanup.")
        else:
            self.cog_print("Tidy_up: Waterboard cog not found. Skipping legacy waterboard DB cleanup.")
        ...
        self.cog_print("Tidy_up: Proceeding with generic temporary channel cleanup by name (Channels with waterdrop emoji').")
        ...
        for channel in current_voice_channels:
            if "💧" in channel.name:  # From original waterboard.py and your tidy_up
                ...
                self.cog_print(f"Tidy_up (name check): Deleting channel: {channel_name_safe}")
                try:
                    await channel.delete(reason="Tidy Up command (name match '💧')")
                    generic_deleted_count += 1
                except nextcord.errors.NotFound:
                    self.cog_print(f"Tidy_up (name check): Channel {channel_name_safe} was already deleted.")
                except Exception as e:
                    self.cog_print(f"Tidy_up (name check): Error deleting channel {channel_name_safe}: {e}")
        ...
        self.cog_print(f"Tidy_up: Generic name-based cleanup deleted {generic_deleted_count} channels.")
        ...
        self.cog_print(f"Tidy_up: {interaction.user.name} ran tidy_up command.")
    ...
    async def reserve_channel(
        ...
        self.cog_print(f"{interaction.user.name} reserved channel {channel.name} for {duration} {unit}.")
    ...
    async def move_channel_to_hidden(self, guild, channel, delay):
        ...
        if hidden_category:
            ...
            self.cog_print(f"Moved channel {channel.name} to hidden category after reservation time.")
    ...
    async def create_temp_channel(self, interaction: nextcord.Interaction, name: str):
        ...
        self.cog_print(f"{interaction.user.name} created temporary voice channel: {name}")
        ...
        while True:
            ...
            if check_empty_channel(temp_channel):
                ...
                self.cog_print(f"Deleted temporary voice channel: {name}")
                break
    ...
    async def league(self, interaction: nextcord.Interaction):
        ...
        self.cog_print(f"League channel '{league_channel.name}' moved to seen category.")
